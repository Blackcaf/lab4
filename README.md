...
Используется **гарвардская архитектура**: память команд и память данных разделены и имеют независимые адресные пространства.
...
- **Адресация** -- словенная (один адрес = одно 32-битное слово).
...
Каждая инструкция выполняется за фиксированное число тактов (включая fetch-цикл из 4 микрокоманд).

| Инструкция | Тип | Описание | Тактов |
|------------|-----|----------|--------|
| `NOP` | Sys | Нет операции | 5 |
| `HALT` | Sys | Останов процессора | 5 |
| `ADD Rd, Rs, Rt` | R | Rd = Rs + Rt | 9 |
| `SUB Rd, Rs, Rt` | R | Rd = Rs - Rt | 9 |
| `MUL Rd, Rs, Rt` | R | Rd = Rs * Rt | 9 |
| `DIV Rd, Rs, Rt` | R | Rd = Rs / Rt | 9 |
| `MOD Rd, Rs, Rt` | R | Rd = Rs % Rt | 9 |
| `AND Rd, Rs, Rt` | R | Rd = Rs & Rt | 9 |
| `OR Rd, Rs, Rt` | R | Rd = Rs \| Rt | 9 |
| `XOR Rd, Rs, Rt` | R | Rd = Rs ^ Rt | 9 |
| `CMP Rd, Rs, Rt` | R | Rd = (Rs == Rt ? 1 : 0) | 9 |
| `SHL Rd, Rs, Rt` | R | Rd = Rs << Rt | 9 |
| `SHR Rd, Rs, Rt` | R | Rd = Rs >> Rt (арифм.) | 9 |
| `ADDI Rt, Rs, imm` | I | Rt = Rs + signext(imm) | 9 |
| `ORI Rt, Rs, imm` | I | Rt = Rs \| zeroext(imm) | 9 |
| `LUI Rt, imm` | I | Rt = imm << 16 | 8 |
| `LOAD Rt, imm(Rs)` | I | Rt = Mem[Rs + signext(imm)] | 11 |
| `STORE Rt, imm(Rs)` | I | Mem[Rs + signext(imm)] = Rt | 11 |
| `JZ Rt, imm` | I | if (zero_flag) PC += imm | 7 |
| `JNZ Rt, imm` | I | if (!zero_flag) PC += imm | 7 |
| `IN Rt, port` | I | Rt = порт(port) | 7 |
| `OUT Rs, port` | I | порт(port) = Rs | 7 |
| `PUSH Rs` | S | SP -= 1; Mem[SP] = Rs | 13 |
| `POP Rt` | S | Rt = Mem[SP]; SP += 1 | 13 |
| `JMP addr` | J | PC = addr | 6 |
| `CALL addr` | J | SP -= 1; Mem[SP] = PC; PC = addr | 14 |
| `RET` | J | PC = Mem[SP]; SP += 1 | 16 |
...
2. `INSTR_READ` -- чтение из I-CACHE по адресу MAR → MDR (может вызвать stall при miss)
3. `LATCH_PC_INC` -- PC += 1 (подготовка к следующей инструкции)
4. `LATCH_IR` -- IR_REG = MDR (загрузка и декодирование инструкции)
...
2. **Инжекция встроенных подпрограмм**: 23 стандартных слова Forth (EMIT, KEY, DUP, SWAP, +, -, и др.) компилируются как подпрограммы на RISC-инструкциях в начало потока команд.
...
| `PC` | 32 бита | Счётчик команд (word-address) |
...
1. **Тактовая модель**: каждый вызов `tick()` выполняет одну микрооперацию. Инструкция завершается за фиксированное число тактов (от 5 для NOP до 17 для CALLR).

2. **Кеши**: чтение/запись через direct-mapped кеш. При miss -- stall на 9 тактов (10 тактов總 - 1 текущий).

3. **Стеки**: два независимых стека:
   - **Стек данных** (`data_sp`): начальное значение `MEMORY_SIZE - 5000`, растёт вниз. Используется для PUSH/POP.
   - **Стек вызовов** (`SP`): начальное значение `MEMORY_SIZE - 1000`, растёт вниз. Используется для CALL/RET.
...
- исходный код программы (ссылка на `.f` файл);
- входные данные (`in_stdin`);
- конфигурацию (`in_limit`, `in_cache_size`);
- машинный код (SHA256, base64);
- листинг с мнемониками;
- ожидаемый stdout;
- журнал работы процессора (ограниченный по длине).
