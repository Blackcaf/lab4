...
Forth использует **словарный** механизм. Все слова хранятся в глобальном словаре. Пользовательские слова, определённые через `: name ... ;`, доступны для вызова из последующего кода. Переменные, созданные через `VARIABLE` или `CREATE ... ALLOT`, также глобальны.
...
- **Память команд** -- 16K слов (64 КБ), адресное пространство `0x0000..0xFFFF`.
- **Память данных** -- 16K слов (64 КБ), адресное пространство `0x0000..0xFFFF`.
- **Память микропрограмм** -- 64 ячейки (адреса 0x00..0x3F), каждая хранит список микроопераций.
...
|----------|----------|
| Тип | Direct-mapped |
| Размер | Настраиваемый (по умолчанию 32 строки) |
| Блок | 1 слово |
...
#### Схема работы direct-mapped кеша

![Схема кеша](img/cache.png)

**Элементы схемы:**

| Блок | Описание |
|------|----------|
| MAR | Регистр адреса памяти, содержит `addr[31:0]` |
| Splitter | Разбиение адреса на `tag [31:n+2]` и `index [n+1:2]` |
| Decoder | Декодирует index, выбирает строку в Cache Array |
| Cache Array (32) | 32 строки, каждая: Valid (1 бит) + Tag (25 бит) + Data (32 бита) |
| Comparator | Сравнивает tag из адреса с tag из строки кеша. Выход: hit/miss |
| MUX | Выбирает data: 0 = из кеша (hit), 1 = из памяти (miss) |
| Main Memory | Основная память (16K слов, 32 бита) |
| MDR | Регистр данных памяти |
| Control Logic | Управление записью (we) при hit/miss |

**Поток данных при чтении:**

1. `MAR` → `addr[31:0]` → `Splitter`
2. `Splitter` → `index [n+1:2]` → `Decoder` → `Cache Array` (выбор строки)
3. `Splitter` → `tag [31:n+2]` → `Comparator`
4. `Cache Array` → `tag out` → `Comparator`
5. `Comparator` определяет hit/miss:
   - **Hit**: `Cache Array` → `data out(hit)` → `MUX` → `MDR` (1 такт)
   - **Miss**: `Main Memory` → `data out(miss)` → `MUX` → `MDR` (10 тактов). Также `Main Memory` → `data in` → `Cache Array` (заполнение строки)

**Поток данных при записи (write-through):**

1. `MAR` → `addr` → `Splitter` → `Comparator`
2. `Control Logic` выдаёт `we`:
   - **Hit**: обновляется data в Cache Array + запись в Main Memory
   - **Miss**: запись только в Main Memory (кеш не обновляется)
...
`N = log2(cache_size)`. Для 32 строк: TAG=25 бит, INDEX=5 бит, offset=2 бита.
...
- **Память**: гарвардская, 16K слов для команд и 16K слов для данных.
...
Пример: `ADD R3, R4, R5` → opcode=0x10, rs=4, rt=5, rd=3 → `0x40851800`
...
- **Microcoded**: каждая инструкция выполняется последовательностью микрокоманд из памяти микропрограмм (`MicrocodeMemory`).

## Микрокоманды и Control Unit

### Память микропрограмм

Микрокод хранится в отдельном классе `MicrocodeMemory` (`core/microcode.py`), который реализует интерфейс памяти:

- `write(addr, data)` — запись микропрограммы по адресу (opcode) при инициализации
- `read(addr)` — чтение микропрограммы по адресу (один порт за такт)
- `tick()` — сброс флага порта каждый такт

Адресация: адрес = значение Opcode (0x00..0x36), данные = список микроопераций `list[MicroOp]`.

### Микрооперации

Процессор управляется памятью микропрограмм. Каждая инструкция ISA представлена в виде последовательности микроопераций. Всего 38 типов микроопераций:
...
2. **Кеши**: чтение/запись через direct-mapped кеш. При miss -- stall на 9 тактов (10 тактов - 1 текущий).
